...
my $leaguequery = "select distinct level from batters";
my $playerquery = "select age, pa, level from batters where level = ? and age < 28";
my $isopquery = "select sum(ab) as ab, sum(doubles) as doubles, sum(triples) as triples, sum(hr) as hr, sum(so) as so, sum(pa) as pa from batters where level=? and age < 28 group by level";
my $wobaquery = "select sum(pa) as pa, sum(h) as h, sum(bb) as bb, sum(doubles) as doubles, sum(triples) as triples, sum(hr) as hr from batters where level=? and age < 28 group by level";
my $agequery = "insert into ages (age, stddev, isop, woba, krate, level) VALUES (?,?,?,?,?,?)";
