...
my $adj_isop_query = "select b.uid, b.name, b.league, b.age, b.pa, (s.age_adjusted_isop * (1 - s.krate) * (1 - s.krate)) as age_adjusted_isop, s.isop, s.krate from batters b, stats s where $leaguestring b.uid=s.uid and b.pa > $pa order by age_adjusted_isop desc limit $limit";
my $adj_woba_query = "select b.uid, b.name, b.league, b.age, b.pa, (s.age_adjusted_woba * (1 - s.krate) * (1 - s.krate)) as age_adjusted_woba, s.woba, s.krate from batters b, stats s where $leaguestring b.uid=s.uid and b.pa > $pa order by age_adjusted_woba desc limit $limit";
my $adj_comb_query = "select b.uid, b.name, b.league, b.age, b.pa, (((s.age_adjusted_woba/2) + s.age_adjusted_isop) * (1 - s.krate) * (1 - s.krate)) as comb, s.age_adjusted_isop, s.age_adjusted_woba from batters b, stats s where $leaguestring b.uid=s.uid and b.pa > $pa order by comb desc limit $limit";
...
my $both_isop_query = "select b.uid, b.name, b.league, b.age, b.pa, (s.both_adjusted_isop * (1 - s.krate) * (1 - s.krate)) as both_adjusted_isop from batters b, stats s where $leaguestring b.uid=s.uid and b.age < 28 and b.pa > $pa order by both_adjusted_isop desc limit $limit";
my $both_woba_query = "select b.uid, b.name, b.league, b.age, b.pa, (s.both_adjusted_woba * (1 - s.krate) * (1 - s.krate)) as both_adjusted_woba from batters b, stats s where $leaguestring b.uid=s.uid and b.age < 28 and b.pa > $pa order by both_adjusted_woba desc limit $limit";
my $both_comb_query = "select b.uid, b.name, b.league, b.age, b.pa, (((s.both_adjusted_woba/2) + s.both_adjusted_isop) * (1 - s.krate) * (1 - s.krate)) as comb, s.both_adjusted_isop, s.both_adjusted_woba, s.krate from batters b, stats s where $leaguestring b.uid=s.uid and b.pa > $pa order by comb desc limit $limit";
...
print "<table class=\"pure-table pure-table-horizontal\"><thead><tr><th>Name</th><th>League</th><th>Age</th><th>PA</th><th>Adj BB Rate</th><th>BB Rate</th></tr></thead>\n";
