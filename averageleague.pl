...
my $leaguequery = "select distinct league from batters";
my $playerquery = "select ab, pa, h, bb, doubles, triples, hr, league, so from batters where league = ? and age < 28";
my $isopquery = "select sum(ab) as ab, sum(doubles) as doubles, sum(triples) as triples, sum(hr) as hr, sum(pa) as pa, sum(so) as so from batters where league=? and age < 28 group by league";
my $wobaquery = "select sum(pa) as pa, sum(h) as h, sum(bb) as bb, sum(doubles) as doubles, sum(triples) as triples, sum(hr) as hr from batters where league=? and age < 28 group by league";
